...
def load_data(select_num, name1):  # select_num = 500 #选择每类选取的故障样本数
    ...
    train_labels = train_data[:, 0]  # 7种故障类型标签
...
# 训练集：select_num*7（真实故障） 测试集：3500(真实故障)
# 真实故障样本
true_fault_data_train = np.loadtxt("train_data"+str(select_number)+"_level"+str(level_num)+"_normalization.txt")
true_fault_label_train = np.loadtxt("train_labels"+str(select_number)+".txt")  # 共select_num*7个
...
#                                                         name1="./data_results/NormaldataLevel"+str(level_num)+"total-3500.txt")
true_fault_data_test = np.loadtxt("./data_results/RealFaultdataLevel"
                                  + str(level_num)+"total-3500.txt")  # 已通过load_data(select_num, name1)归一化
true_fault_label_test = np.loadtxt("./data_results/RealFaultlabeltotal-3500.txt")

# Train 训练集：select_num*7（真实故障）
