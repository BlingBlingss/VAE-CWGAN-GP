...
def CHOSE():
    ...
    while choose_label.shape[0] <= 7000:
        ...
        if choose_label.shape[0] == 0:
            break
    np.savetxt(".\data_results\CWAGN_data"+str(select_number)+"_Level"+str(level_num)+"SelectedData_Ensemble.txt", choose_data_all)
...
def classify(x_train1, y_train1, i):
    ...
    min, max = get_min_max()
    x_test, y_test = load_data(select_number, min, max) # 重新加载其他真实数据测试，使用原本真实数据的最小值与最大值归一化
    ...
    print("===== ensemble evaluation %d/30=======" % (i+1))
    print('Accuracy:')
